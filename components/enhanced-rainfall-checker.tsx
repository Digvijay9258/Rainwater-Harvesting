...
import { CloudRain, MapPin, Droplets, Clock, TrendingUp, Award, Globe, Calculator } from "lucide-react"
import {
  PROPERTY_TYPES,
...
export default function EnhancedRainfallChecker() {
  ...
  useEffect(() => {
    if (weatherData?.annualRainfall) {
      calculateAreaBasedHarvesting()
    }
  }, [selectedPropertyType, customArea, weatherData])
  ...
  const calculateAreaBasedHarvesting = () => {
    if (!weatherData?.annualRainfall) return

    const customAreaNum = customArea ? Number.parseFloat(customArea) : null
    const calculation = calculateRainwaterHarvesting(selectedPropertyType, customAreaNum, weatherData.annualRainfall)
      ...
      <Card className="border-blue-200 bg-gradient-to-br from-blue-50 to-cyan-50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-blue-900">
            ...
            Enhanced Rainfall & Harvesting Calculator
          </CardTitle>
          <CardDescription className="text-blue-700">
            Check rainfall data and calculate rainwater harvesting potential for different property types
          </CardDescription>
        </CardHeader>
        ...
        <Card className="border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
          <CardHeader>
            ...
            <CardDescription className="text-green-700">
              Calculate harvesting potential based on your property type and area
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">
                  Roof Area (m²)
                  <span className="text-gray-500 font-normal">(Default: {selectedPropertyType.avgRoofArea}m²)</span>
                </label>
                <Input
                  type="number"
                  placeholder={`Default: ${selectedPropertyType.avgRoofArea}m²`}
                  value={customArea}
              ...
              <div className="grid gap-4 md:grid-cols-3 mt-6">
                <Card className="bg-blue-50 border-blue-200">
                  <CardContent className="p-4 text-center">
                    ...
                    <div className="text-sm text-blue-700">Daily Collection</div>
                  </CardContent>
                </Card>

                <Card className="bg-green-50 border-green-200">
                  <CardContent className="p-4 text-center">
                    <div className="text-2xl font-bold text-green-600">{areaCalculation.waterSavings}%</div>
                    <div className="text-sm text-green-700">Water Demand Met</div>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-purple-50 border-purple-200">
                  <CardContent className="p-4 text-center">
                    <div className="text-2xl font-bold text-purple-600">
                      ${areaCalculation.costSavings.toLocaleString()}
                    </div>
            ...
            {areaCalculation?.recommendations && (
              <Card className="bg-amber-50 border-amber-200">
                <CardHeader>
                  <CardTitle className="text-amber-900 text-lg">Recommendations</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="space-y-2">
                    {areaCalculation.recommendations.map((rec, index) => (
            ...
            <CardContent className="space-y-4">
              ...
              {weatherData.annualRainfall && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-md">
                  ...
                  <p className="text-lg font-bold text-blue-600">{weatherData.annualRainfall.toLocaleString()}mm</p>
                </div>
              )}
